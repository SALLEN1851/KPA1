...
if(!( function_exists('ebor_add_body_classes') )){ 
	function ebor_add_body_classes($classes) {
        ...
        if( 'yes' == get_option('stack_use_vertical_responsive', 'no') && !( 'vertical' == ebor_get_header_layout() ) ) {
			$classes[] = 'vertical-nav-on-mobile';
		}
...
if(!( function_exists( 'ebor_single_oembed_result' ) )){
	function ebor_single_oembed_result( $html, $url, $args ) {
		...
		if( 'no' == get_option( 'stack_single_post_embed_cover', 'yes' ) ){
			return $html;
		}
